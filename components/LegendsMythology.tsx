...
import { generateMythology } from '../services/geminiService';
...
interface LegendsMythologyProps {
    birthData: BirthData;
}

export const LegendsMythology: React.FC<LegendsMythologyProps> = ({ birthData }) => {
    ...
    const handleGenerate = useCallback(async () => {
        ...
        setContent('');
        try {
            const result = await generateMythology(birthData, language);
            setContent(result);
        } catch (err) {
    ...
    return (
        <div className="card-base p-6 sm:p-8 rounded-2xl space-y-6">
            <div className="text-center">
                <h2 className="text-3xl font-bold celestial-title">{t.legendsMythology.title}</h2>
                <p className="text-gray-400 mt-2">{t.legendsMythology.description}</p>
            </div>
                ...
                <div className="animate-fade-in space-y-4">
                    <div className="text-gray-300 leading-relaxed p-4 bg-black/20 rounded-lg">
                     ...
                     <div className="flex flex-col sm:flex-row pt-4 gap-4">
                        ...
                        <ShareButton shareText={content} shareTitle={t.legendsMythology.title} />
                    </div>
                </div>
            )}
        </div>
